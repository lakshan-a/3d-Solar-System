...
const NeptuneVisualization = () => {
    const canvasRef = useRef(null);
    const [rotationSpeed, setRotationSpeed] = useState(0.5);
    const [viewMode, setViewMode] = useState('neptune');
    ...
    const [stats] = useState({
        ...
        windSpeeds: 'Up to 2,100 km/h (fastest in solar system)',
        uniqueFeature: 'Great Dark Spot (storm system)'
    });
    ...
    const neptune = useRef(new THREE.Group());
    const composer = useRef(null);
    const bloomPass = useRef(null);
    const outputPass = useRef(null);
    const neptuneRotationSpeed = useRef(0.0005);
    ...
    // Texture paths for Neptune
    const texturePaths = useMemo(() => ({
        colorMap: neptuneTexture
    }), []);
    ...
    useEffect(() => {
        ...
        const init = () => {
            ...
            // Lighting - cooler light for Neptune
            directionalLight.current = new THREE.DirectionalLight(0x99ccff, lightIntensity);
            ...
            hemiLight.current = new THREE.HemisphereLight(0x224477, 0x000011, 0.3);
        ...
        const createNeptune = () => {
            scene.current.add(neptune.current);

            // Neptune geometry
            const neptuneGeometry = new THREE.SphereGeometry(2, 256, 256);

            // Neptune material - deep blue colors
            const neptuneMaterial = new THREE.MeshPhysicalMaterial({
                roughness: 0.8,
                metalness: 0.1,
                clearcoat: 0.05,
                clearcoatRoughness: 0.3,
                color: 0x1a4d8c, // Deep blue base color
                specularColor: new THREE.Color(0x3366cc),
                specularIntensity: 0.5,
                sheen: 0.2,
                sheenColor: new THREE.Color(0x4477dd)
            });

            // Load textures with deep blue color processing
            ...
            // Apply Neptune's axial tilt (28.32°)
            neptuneMesh.rotation.z = THREE.MathUtils.degToRad(28.32);

            neptune.current.add(neptuneMesh);

            // Create atmosphere with deep blue color
            const atmosphereGeometry = new THREE.SphereGeometry(2.1, 128, 128);
            const atmosphereMaterial = new THREE.MeshPhysicalMaterial({
                color: 0x2a5d9c, // Deep blue atmosphere color
                transparent: true,
                opacity: 0.25,
                roughness: 0.7,
                metalness: 0,
                ...
                specularColor: new THREE.Color(0x4477dd),
                specularIntensity: 0.5,
                clearcoat: 0.1,
                emissive: 0x1a3d7c,
                emissiveIntensity: 0.1,
                sheen: 0.3,
                sheenColor: new THREE.Color(0x3366cc)
            });
            ...
            atmosphereMesh.current.rotation.z = THREE.MathUtils.degToRad(28.32);
            neptune.current.add(atmosphereMesh.current);
        };
        ...
        const animate = () => {
            ...
            // Rotate Neptune
            neptune.current.rotation.y += neptuneRotationSpeed.current;
            ...
            bloomPass.current.enabled = bloomEnabled;
            if (bloomEnabled) {
                bloomPass.current.strength = 0.8 + Math.sin(Date.now() * 0.001) * 0.1; // Subtle bloom
            }
    ...
    useEffect(() => {
        neptuneRotationSpeed.current = rotationSpeed * 0.001;
    }, [rotationSpeed]);
                ...
                <div className="absolute top-4 left-4 right-4 bottom-4 md:top-6 md:left-6 md:right-auto md:bottom-auto info-panel z-10 bg-gray-900/95 backdrop-blur-md rounded-2xl border border-gray-800 shadow-2xl transition-all duration-300 overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                        <p className="text-gray-400 mb-4 text-sm italic">
                            "The windiest planet with the strongest winds in the solar system, featuring a deep blue color from methane in its atmosphere."
                        </p>
...
export default NeptuneVisualization;