...
const UranusVisualization = () => {
    const canvasRef = useRef(null);
    const [rotationSpeed, setRotationSpeed] = useState(0.5);
    const [viewMode, setViewMode] = useState('uranus');
    ...
    const [stats] = useState({
        ...
        pressure: 'Unknown (ice giant)',
        ringSystem: '13 faint rings',
        discovery: '1781 by William Herschel',
        mass: '14.5 Earth masses',
        magneticField: 'Tilted 59° from axis',
        axialTilt: '97.77° (sideways rotation)',
        composition: 'Icy mantle, rocky core',
        windSpeeds: 'Up to 900 km/h',
        uniqueFeature: 'Extreme axial tilt'
    });
    ...
    const uranus = useRef(new THREE.Group());
    const composer = useRef(null);
    const bloomPass = useRef(null);
    const outputPass = useRef(null);
    const uranusRotationSpeed = useRef(0.0005);
    ...
    // Texture paths for Uranus and its rings
    const texturePaths = useMemo(() => ({
        colorMap: uranusTexture,
    ...
    useEffect(() => {
        ...
        const init = () => {
            ...
            // Lighting - cooler light for Uranus
            directionalLight.current = new THREE.DirectionalLight(0xccddff, lightIntensity);
            ...
            // Ambient light with cooler color
            hemiLight.current = new THREE.HemisphereLight(0x334477, 0x000011, 0.3);
            scene.current.add(hemiLight.current);

            // Create Uranus with rings
            createUranus();
        ...
        const createUranus = () => {
            scene.current.add(uranus.current);

            // Uranus geometry
            const uranusGeometry = new THREE.SphereGeometry(2, 256, 256);

            // Uranus material - cooler colors
            const uranusMaterial = new THREE.MeshPhysicalMaterial({
                roughness: 0.9,
                metalness: 0.1,
                clearcoat: 0.05,
                clearcoatRoughness: 0.3,
                color: 0xc1e0ff, // Cool cyan-blue base color
                specularColor: new THREE.Color(0xaaccff),
                specularIntensity: 0.4
            });

            // Load textures with cooler color processing
            ...
            // Apply Uranus' extreme axial tilt (97.77°)
            uranusMesh.rotation.z = THREE.MathUtils.degToRad(97.77);

            uranus.current.add(uranusMesh);

            // Create atmosphere with cooler color
            const atmosphereGeometry = new THREE.SphereGeometry(2.1, 128, 128);
            const atmosphereMaterial = new THREE.MeshPhysicalMaterial({
                color: 0xb5d5ff, // Cool cyan-blue atmosphere color
                transparent: true,
                opacity: 0.2,
                roughness: 0.8,
                metalness: 0,
            ...
            // Match the axial tilt
            atmosphereMesh.current.rotation.z = THREE.MathUtils.degToRad(97.77);
            uranus.current.add(atmosphereMesh.current);

            // Create rings with cooler colors
            textureLoader.current.load(texturePaths.ringTexture, (texture) => {
                texture.anisotropy = renderer.current.capabilities.getMaxAnisotropy();
                texture.encoding = THREE.sRGBEncoding;

                // Main rings - cooler colors
                ...
                const ringMaterial = new THREE.MeshStandardMaterial({
                    ...
                    metalness: 0.2,
                    roughness: 0.8,
                    color: 0xaabbcc, // Cooler ring color
                    emissive: 0x334455,
                ...
                // Match Uranus' axial tilt for rings
                rings.current.rotation.z = THREE.MathUtils.degToRad(97.77);
                rings.current.name = "Rings";
                uranus.current.add(rings.current);

                // Ring shadows on Uranus
                if (showRingShadows) {
                    ...
                    // Match axial tilt
                    ringShadows.current.rotation.z = THREE.MathUtils.degToRad(97.77);
                    ringShadows.current.name = "RingShadows";
                    uranus.current.add(ringShadows.current);
                }
            });
        };
        ...
        const animate = () => {
            ...
            // Rotate Uranus
            uranus.current.rotation.y += uranusRotationSpeed.current;

            // Rotate rings
            if (rings.current) {
                rings.current.rotation.y += uranusRotationSpeed.current * 0.2; // Slower ring rotation
            }
            ...
            if (atmosphereMesh.current) {
                atmosphereMesh.current.visible = showAtmosphere;
                atmosphereMesh.current.material.opacity = showAtmosphere ? 0.2 : 0;
            ...
            bloomPass.current.enabled = bloomEnabled;
            if (bloomEnabled) {
                bloomPass.current.strength = 1.0 + Math.sin(Date.now() * 0.001) * 0.1; // More subtle bloom
            }
    ...
    useEffect(() => {
        uranusRotationSpeed.current = rotationSpeed * 0.001;
    }, [rotationSpeed]);
    ...
    useEffect(() => {
        ...
        switch (viewMode) {
            case 'uranus':
                ...
                break;
            case 'northPole':
                // Adjusted for Uranus' extreme tilt
            ...
            case 'darkSide':
                camera.current.position.set(-10, 0, 0);
                camera.current.lookAt(0, 0, 0);
                setLightIntensity(0.4); // Darker for Uranus
                break;
            case 'ringsView':
                // Shows Uranus' faint rings
                ...
                <div className="absolute top-4 left-4 right-4 bottom-4 md:top-6 md:left-6 md:right-auto md:bottom-auto info-panel z-10 bg-gray-900/95 backdrop-blur-md rounded-2xl border border-gray-800 shadow-2xl transition-all duration-300 overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                        <p className="text-gray-400 mb-4 text-sm italic">
                            "The ice giant with extreme axial tilt, rotating on its side with a pale blue-green hue from methane in its atmosphere."
                        </p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6 stats-grid">
                            {Object.entries(stats).map(([key, value]) => (
                                <div key={key} className="bg-gray-800/90 p-3 rounded-lg border-l-2 border-blue-500/50 hover:border-blue-400 transition-colors">
                                    <div className="text-blue-400 text-xs font-medium uppercase tracking-wider">{key.replace(/([A-Z])/g, ' $1')}</div>
                        ...
                        <div className="space-y-4 pb-4">
                            {/* Sliders */}
                            {[
                                { label: 'Rotation Speed', value: rotationSpeed, min: 0, max: 2, step: 0.1, onChange: setRotationSpeed, color: 'blue' },
                                { label: 'Sun Intensity', value: lightIntensity, min: 0.1, max: 3, step: 0.1, onChange: setLightIntensity, color: 'blue' },
                                { label: 'Exposure', value: exposure, min: 0.1, max: 1.5, step: 0.1, onChange: setExposure, color: 'blue' },
                                { label: 'Ring Opacity', value: ringOpacity, min: 0, max: 1, step: 0.1, onChange: setRingOpacity, color: 'cyan' },
                            ].map((slider) => (
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <button
                                    ...
                                    className={`py-2 px-1 rounded-lg text-sm transition-all flex items-center justify-center ${
                                        showAtmosphere
                                            ? 'bg-cyan-700/90 text-white shadow-md shadow-cyan-500/20'
                                            : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                                    }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`py-2 px-1 rounded-lg text-sm transition-all flex items-center justify-center ${
                                        showRingShadows
                                            ? 'bg-indigo-700/90 text-white shadow-md shadow-indigo-500/20'
                                            : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                                    }`}
                                >
...
export default UranusVisualization;